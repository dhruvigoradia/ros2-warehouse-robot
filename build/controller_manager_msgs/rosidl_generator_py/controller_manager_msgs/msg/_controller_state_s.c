...
bool controller_manager_msgs__msg__controller_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // update_rate
    ...
    ros_message->update_rate = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // claimed_interfaces
  ...
  {  // exported_state_interfaces
    PyObject * field = PyObject_GetAttrString(_pymsg, "exported_state_interfaces");
    if (!field) {
      return false;
    }
    {
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'exported_state_interfaces'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
      ...
      rosidl_runtime_c__String * dest = ros_message->exported_state_interfaces.data;
      for (Py_ssize_t i = 0; i < size; ++i) {
        PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
        if (!item) {
          Py_DECREF(seq_field);
...
PyObject * controller_manager_msgs__msg__controller_state__convert_to_py(void * raw_ros_message)
{
  ...
  {  // update_rate
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->update_rate);
    {
  ...
  {  // exported_state_interfaces
    ...
    rosidl_runtime_c__String * src = ros_message->exported_state_interfaces.data;
    field = PyList_New(size);
    if (!field) {
      return NULL;
    }
    ...
    assert(PySequence_Check(field));
    {
      int rc = PyObject_SetAttrString(_pymessage, "exported_state_interfaces", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
